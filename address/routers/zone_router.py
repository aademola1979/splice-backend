...
from address.controller.zone_controller import ZoneController
...
async def get_all_zones(session: AsyncSession = Depends(get_session)):
    zones = await controller.get_all_zone(session=session)
    return zones
...
async def create_new_zone(zone_data:CreateZoneSchema, session: AsyncSession = Depends(get_session))->dict:
    new_zone = await controller.create_zone(zone_data=zone_data, session=session)
    return new_zone
...
async def get_zone_by_id(zone_id:UUID, session: AsyncSession = Depends(get_session)):
    zone = await controller.get_zone_by_id(zone_id=zone_id, session = session)
    if zone is None:
...
async def update_zone(zone_id:UUID, update_data:UpdateZoneSchema, session: AsyncSession = Depends(get_session)):
    zone = await controller.update_zone(zone_id=zone_id, update_data=update_data, session=session)
    if zone is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Zone not found")
    return zone
...
async def delete_user(zone_id:UUID, session: AsyncSession = Depends(get_session)):
    message: Union[str, None] = await controller.delete_zone(zone_id=zone_id, session=session)
