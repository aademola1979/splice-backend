...
async def get_all_users(session: AsyncSession = Depends(get_session)):
    users = await service.get_all_user(session=session)
    return users 
...
async def create_new_user(user_data:CreateUserShcema, session: AsyncSession = Depends(get_session))->dict:
    new_user = await service.create_user(user_data=user_data, session=session)
    return new_user
...
async def update_user(user_id:UUID, update_data:UpdateUserSchema, session: AsyncSession = Depends(get_session)):
    user = await service.update_user(user_id=user_id, update_data=update_data, session=session)
    if user is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
    return user
...
async def delete_user(user_id:UUID, session: AsyncSession = Depends(get_session)):
    user = await service.delete_user(user_id=user_id, session=session)
